...
function SignUpPage() {
  ...
  const onSbmt = async () => {
    try {
      const value = await createUserWithEmailAndPassword(auth, createUsername, createPassword);
  ...
  return (
    <Card
      ...
      onUsernameChange={(e) => setCreatedUsername(e.target.value)}
      onPassWordChange={(e) => setCreatedPassword(e.target.value)}
      onSbmt={onSbmt}
    />
  )
}
